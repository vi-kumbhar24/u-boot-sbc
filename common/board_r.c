...
#ifdef CONFIG_SECO_SYSTEM_DATA
static int sysdata_seco_switchpart_check( void ) {
	int ret = sysdata_integration_check( );

	printf( "System data from %s... ", get_sysdata_device_name( ) );
	if ( ret == 0 )
		printf( "OK.\n" );
	else if ( ret == 1)
		printf( "Inizializated.\n" );
	else {
		printf( "Error, not available\n" );
		return 0;
	}

	ret = sysdata_switchpart_check( );
	if ( ret < 0 )
		printf( "Unable to read switchpart data\n" );
	else if ( ret > 0 )
		printf( "Reach num max of fail boot. Switched to part group %d\n", ret );
