...
int dram_init(void)
{
	...
	end2 = (sc_faddr_t)PHYS_SDRAM_2 + sdram_size;
        ...
        unsigned long long sdram1_size = 0x00000000;
		unsigned long long sdram2_size = 0x00000000;

        if(C57_IS_1GB){
			gd->seco_cfg = RAM_1GB;
			sdram1_size = PHYS_DRAM_IS_1GB;
			sdram2_size = 0x00000000;
		}
        else if(C57_IS_2GB){
			gd->seco_cfg = RAM_2GB;
			sdram1_size = PHYS_DRAM_IS_2GB;
			sdram2_size = 0x00000000;
		}
        else if(C57_IS_4GB){
			gd->seco_cfg = RAM_4GB;
			sdram1_size = PHYS_DRAM_IS_2GB;
			sdram2_size = PHYS_DRAM_IS_2GB;
		}

        end1 = (sc_faddr_t)PHYS_SDRAM_1 + sdram1_size;
        end2 = (sc_faddr_t)PHYS_SDRAM_2 + sdram2_size;
...
int dram_init_banksize(void)
{
	...
	unsigned long long sdram_size;
        ...
        if(gd->seco_cfg == RAM_1GB){
			sdram1_size = PHYS_DRAM_IS_1GB;
			sdram2_size = 0x00000000;
		}
        else if(gd->seco_cfg == RAM_2GB){
			sdram1_size = PHYS_DRAM_IS_2GB;
			sdram2_size = 0x00000000;
		}
        else if(gd->seco_cfg == RAM_4GB){
			sdram1_size = PHYS_DRAM_IS_2GB;
			sdram2_size = PHYS_DRAM_IS_2GB;
		}
...
static u64 get_block_attrs(sc_faddr_t addr_start)
{
	...
	if ((addr_start >= PHYS_SDRAM_1 &&
        ...
        if ((addr_start >= PHYS_SDRAM_1 &&
             addr_start <= ((sc_faddr_t)PHYS_SDRAM_1 + sdram1_size)) ||
            (addr_start >= PHYS_SDRAM_2 &&
             addr_start <= ((sc_faddr_t)PHYS_SDRAM_2 + sdram2_size)))
#else
