...
static void board_gpio_init(void)
{
	...
	/* Put WiFi in powerdown */
        ret = dm_gpio_lookup_name("gpio@20_7", &desc);
        if (ret)
                return;

        ret = dm_gpio_request(&desc, "wifi_pwr_down");
        if (ret)
                return;
...
{
	int ret = 0;
	struct gpio_desc usb_hub_desc;
	struct gpio_desc usb_hub_mode;

	ret = dm_gpio_lookup_name("gpio@20_1", &usb_hub_desc);
        if (ret) {
                printf("%s lookup gpio@20_1 failed ret = %d\n", __func__, ret);
                return;
        }
	ret = dm_gpio_request(&usb_hub_desc, "usb-hub-rst");
        if (ret) {
                printf("%s request usb hub rst failed ret = %d\n", __func__, ret);
                return;
        }

	ret = dm_gpio_lookup_name("gpio@20_0", &usb_hub_mode);
        if (ret) {
                printf("%s lookup gpio@20_0 failed ret = %d\n", __func__, ret);
                return;
        }
        ret = dm_gpio_request(&usb_hub_mode, "usb-hub-mode");
        if (ret) {
                printf("%s request usb hub rst failed ret = %d\n", __func__, ret);
                return;
        }

        /* Put out USB hub in mode : configuration from internal ROM */
        dm_gpio_set_dir_flags(&usb_hub_mode, GPIOD_IS_OUT);
	dm_gpio_set_value(&usb_hub_mode, 0);
        /* Put out from reset the hub */
        dm_gpio_set_dir_flags(&usb_hub_desc, GPIOD_IS_OUT);
	dm_gpio_set_value(&usb_hub_desc, 0);
	mdelay(50);
	dm_gpio_set_value(&usb_hub_desc, 1);
	mdelay(50);
	dm_gpio_set_value(&usb_hub_desc, 0);
