...
static int serial_check_stdout(const void *blob, struct udevice **devp, struct udevice **add_uart_devp)
{
	int node;
	...
	/* Additional UART Node */
	add_uart_node = fdtdec_get_chosen_node(blob, "additional-uart");
	if (!uclass_get_device_by_of_offset(UCLASS_SERIAL, add_uart_node, add_uart_devp)) {
		printf("Additional uart: %s \n",(*add_uart_devp)->name);
	}
...
static void serial_find_console_or_panic(void)
{
	...
	struct udevice *dev,*add_uart_dev;
	...
	if (CONFIG_IS_ENABLED(OF_PLATDATA)) {
		...
		if (of_live_active()) {
			...
			if (!serial_check_stdout(blob, &dev, &add_uart_dev)) {
#ifdef CONFIG_CMD_SECO_UART
				gd->add_serial_dev = add_uart_dev;
#endif
				gd->cur_serial_dev = dev;
				return;
			}
...
void serial_dev_putc(char ch, struct udevice *dev)
{
        if (dev)
                _serial_putc(dev, ch);
}
...
void serial_dev_setbrg(int baudrate, struct udevice *dev)
{
	struct dm_serial_ops *ops;
...
const char * serial_dev_getname(struct udevice *dev)
{
	if(dev->name != NULL)
		return dev->name;
